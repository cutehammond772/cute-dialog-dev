...
import { PatchEventSignature, PatchSignature } from "@core/patch/common";
import { HandleEventMappings, PatchEvent, PatchEventCallback } from "@core/patch/event";
import { PatchSubscribe } from "@core/patch/patcher";
...
const usePatchEvents = () => {
  ...
  /**
   * 특정 PatchEvent를 구독합니다.
   *
   * - 그 PatchEvent를 담당하는 Patch의 등록 여부와 상관없이 구독할 수 있습니다.
   * - 단, 컴포넌트 로직의 Top-Level에서 호출해야 합니다. (= Hook의 위치와 동등함)
   * - 한 PatchEvent 당 하나의 콜백 함수만 매핑됩니다.
   ...
   * 예를 들어, "onclick" Event가 발생할 시 이에 매핑되는 PatchEvent가 발생되도록 합니다.
   *
