...
import { Patch, PatchRegisterCallback } from "@core/patch/patcher";
...
const usePatchNodes = () => {
  ...
  const hasPatch = useCallback(
    (signature: PatchSignature) => signatures.current.has(signature),
    []
  );
  ...
  return { hasPatch, reservePatch, onPatchRegister, nodes };
};
